...
const totalRows = employeeTable.rows.length; // Total number of rows
...
function displayPage(page) {
    ...
    for (let i = start; i < end && i < totalRows; i++) {
        employeeTable.rows[i].style.display = '';
    }
...
function updatePagination() {
    ...
    const paginationList = pagination.querySelectorAll('.page-item:not(#prevPage):not(#nextPage)');
    paginationList.forEach(item => item.remove());

    // Display a few page numbers with dots
    const maxPageLinks = 2; // Max number of page links to show before and after current
    let startPage = Math.max(1, currentPage - maxPageLinks); // Start page
    let endPage = Math.min(totalPages, currentPage + maxPageLinks); // End page
...
function addPageLink(page) {
    ...
    li.innerHTML = `<a style="font-size:19px;" class="page-link" href="#">${page}</a>`;
...
// Initial display
displayPage(currentPage);
// Function to remove extra spaces from all elements with class 'employeeName'
